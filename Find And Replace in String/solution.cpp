...
public:
  ...
  string findReplaceString(string S, vector<int>& idx, vector<string>& src, vector<string>& tgs) {
      //  string s1 = "vmokbfrzp";
      //  string s2 = ""jayflmrufcwnrlheoobkmmlitiwjdxsotvznixpghnxsodcieatwspiadgcedgyzrjvfcmchhtiraqiowzrerkofjmyimqdyrscdonpplte"";
      /*  string ans = S;
        for ( int i = 0; i < (int)indexes.size(); i++){
 
            if (S.substr(indexes[i], (int)sources[i].length()) == sources[i]){
        
                int pos = ans.find(sources[i]);              
                if (pos >= 0){
                  ans.replace(ans.begin() + pos ,ans.begin() + pos + (int)sources[i].length(), targets[i]);    
                }
